...
"숲"과 "정글"이 음악적으로 뚜렷이 구분되지 않는 경우가 많아 **6개로 병합**할 것을 제안했고,
이후 논의를 거쳐 **`hell`(지옥/용암)**과 **`space`(우주)** 2개를 추가해 최종 **8개 카테고리**로
확정한다. 필요하면 나중에 세부 태그(하위 라벨)로 다시 쪼개면 된다.
...
| `city_machine` | 현대도시 / 기계 | 자연 바이옴이 아니라 인공물 — 아래 3번 참고 |
| `hell` | 지옥 / 용암 | 자연 바이옴이 아니라 신화적/판타지 테마 — 아래 3번 참고 |
| `space` | 우주 | 자연 바이옴이 아니라 SF 테마 — 아래 3번 참고 |
...
| 지옥/용암 | **Terraria (The Underworld/Hell 바이옴이 이미 있음)**, Hades(게임 전체가 명계, Tartarus/Asphodel/Elysium/Styx 등 챔버별로 뚜렷이 구분되고 Darren Korb OST 호평) | Diablo 시리즈(Hell Act, 위키 매우 체계적), DOOM 시리즈(Hell 레벨, Mick Gordon OST), World of Warcraft(Firelands, Molten Core, Twisting Nether 등) |
| 우주 | **Terraria (Space Day/Night 바이옴이 이미 있음)**, Outer Wilds(게임 전체가 우주 탐사, 행성별 테마가 뚜렷하고 Andrew Prahlow OST 호평) | Mass Effect 시리즈, Destiny 시리즈, Halo 시리즈, FFXIV(Ultima Thule 등) |

**"현대도시/기계", "지옥/용암", "우주" 3개는 나머지 5개와 성격이 다르다.** 나머지는 자연
바이옴이라 대형 오픈월드/MMORPG 한두 개만으로도 100곡을 채우기 쉽지만, 이 3개는 자연
지형이 아니라 인공적·신화적·SF 테마라 해당 게임 자체가 상대적으로 적다. 이 3개는
"게임 수를 늘리는" 전략보다 "테마에 맞는 게임의 개별 던전/구역/챔버 트랙까지 긁어모으는"
전략이 필요하다. 100곡 목표가 가장 늦게 채워질 카테고리들로 예상하고 우선순위를 낮게
잡는 게 현실적이다.
...
- 카테고리당 100곡 × 8카테고리 = 총 800곡 목표는 **forest_jungle / snow_ice / desert_rock /
...
- **city_machine**은 100곡이 빠듯할 수 있다 — 70~80곡 선에서 타협하거나, 조건을 살짝
  ...
  검토가 필요하다.
- **hell**은 Diablo/DOOM 시리즈를 더하면 100곡이 충분히 가능하나, Hades 하나만으로는
  30곡 안팎(챔버 수가 제한적)이라 시리즈 여러 개를 합쳐야 한다.
- **space**는 Mass Effect/Destiny/Halo 등 대형 시리즈를 더하면 100곡 가능하나, Outer Wilds
  단독으로는 20~30곡 수준이라 마찬가지로 여러 게임을 합쳐야 한다.
- 지금 단계에서는 city_machine/hell/space 3개의 우선순위를 가장 낮게 두고 나머지
  5개(자연 바이옴) 카테고리를 먼저 채우는 순서를 권장.
