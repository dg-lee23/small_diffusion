...
#
# 주의: 1단계(위치 트리 파싱)는 실제 위키 구조를 검증 못 하고 짠 최선의 추정이다.
# 먼저 아래처럼 --dry-run으로 소량만 확인해보는 걸 강력히 권장:
#   python3 genshin/scrape_locations.py --dry-run
# 이후 단계(특히 3단계 LLM 분류)는 API 비용이 드니, 1단계 결과가 말이 되는지
# 확인한 뒤에 이 스크립트 전체를 돌릴 것.
set -euo pipefail
